 ...
 */
static int at_parse_param_u32(const char *at_params_str, u32_t *val,
				 size_t *consumed)
{
	u32_t value = 0;
	u32_t check_value;
	bool negative = false;
	const char *at_str = at_params_str;
...
/* Internal function. Parameters cannot be null. String must be null terminated.
 */
static int at_parse_param_numeric(const char * at_params_str,
				       struct at_param_list *const list,
				       size_t index, size_t *consumed)
{
	char *at_str = (char *)at_params_str;
	size_t num_spaces = at_params_remove_spaces_from_beginning(&at_str);
...
/* Internal function. Parameters cannot be null. String must be null terminated.
 */
static int at_parse_param_string(const char *const at_params_str,
				      struct at_param_list *const list,
				      size_t index, size_t *consumed)
{
	u16_t str_len;
	bool in_double_quotes = false;
	char *str = (char *)at_params_str;
	...
	size_t spaces = at_params_remove_spaces_from_beginning(&str);
	...
	param_value_start = str;

	while ((*str != '\0') && ((!in_double_quotes && ((*str != AT_CMD_SEPARATOR)
			|| (*str != AT_CMD_PARAM_SEPARATOR)))
			|| (in_double_quotes && (*str != '\"')))) {
				str++;
	}

	if (in_double_quotes &&
	    ((*str == AT_CMD_SEPARATOR) || (*str == '\0'))) {
		return -EINVAL;
	}
...
/* Internal function. Parameters cannot be null. String must be null terminated.
 */
static int at_parse_param(const char *const at_params_str,
			       struct at_param_list *const list, size_t index,
			       size_t *consumed)
{
...
int at_parser_params_from_str(const char *const at_params_str,
			      struct at_param_list *const list)
{
...
int at_parser_max_params_from_str(const char *const at_params_str,
				  struct at_param_list *const list,
				  size_t max_params_count)
{
	...
	(void)at_params_remove_spaces_from_beginning(&str);

	for (size_t param_idx = 0; param_idx < max_params_count; ++param_idx) {
		size_t consumed;
		int err = at_parse_param(str, list, param_idx, &consumed);
