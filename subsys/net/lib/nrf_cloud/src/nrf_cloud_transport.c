...
static void dc_endpoint_free(void)
{
	...
	if (nct.dc_endp.utf8 != NULL) {
		nrf_cloud_free(nct.dc_endp.utf8);
	}
	dc_endpoint_reset();
}
...
void nct_dc_endpoint_set(const struct nrf_cloud_data *tx_endp,
			 ...
			 const struct nrf_cloud_data *endp)
{
			 ...
			 struct nrf_cloud_data *const endp)
{
	...
	if (endp != NULL) {
		endp->ptr = nct.dc_endp.utf8;
		endp->len = nct.dc_endp.size;
	}
}
